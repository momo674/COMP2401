...
void growArray(StudentArray * stuArray) {
  int new_capacity = stuArray->capacity * 2;
  StudentType* *new_array = (StudentType **) calloc(new_capacity, sizeof(StudentType *));

  for (int i = 0; i < stuArray->count; i++) {
    new_array[i] = stuArray->arr[i];
  }
  free(stuArray->arr);
  stuArray->capacity = new_capacity;
  stuArray->arr = new_array;
